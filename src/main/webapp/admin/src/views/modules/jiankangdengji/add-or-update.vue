<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="省份" prop="shengfen">
          <el-select v-model="ruleForm.shengfen" placeholder="请选择省份">
            <el-option
                v-for="(item,index) in shengfenOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="市区" prop="shiqu">
          <el-select v-model="ruleForm.shiqu" placeholder="请选择市区">
            <el-option
                v-for="(item,index) in shiquOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="其他说明" prop="qitashuoming">
                <el-input
              ...
              <div v-else>
                <el-form-item v-if="ruleForm.qitashuoming" label="其他说明" prop="qitashuoming">
                    <span>{{ruleForm.qitashuoming}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
      ...
      this.$http({
		...
		if(json.xingbie!=''&&json.xingbie){
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/shengfen/shengfen`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.shengfenOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
            ...
            this.$http({
              ...
              if (data && data.code === 0) {
                this.shiquOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
         
            this.shentizhuangkuangOptions = "健康,发烧,干咳,乏力,其他".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
